...
Read today and tell me what it is.`;
  ...
  // Call Claude via GAS proxy using GET — avoids CORS preflight on POST
  const claudePayload = {
  ...
  const proxyURL = `${SAPPHIRA_API}?sapphira=1&payload=${encodeURIComponent(JSON.stringify(claudePayload))}`;
